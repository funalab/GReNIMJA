...
def make_filter_pwm(filter_fasta):
    ...
    if data_type == 'DNA':
        ...
        return np.array(pwm_freqs), nsites
    else:
        #nsites = 20  # pseudocounts
        nsites = 0  # pseudocounts
        for line in open(filter_fasta):
            if line[0] != '>':
                seq = line.rstrip()
                ...
                if len(pwm_counts) == 0:
                    # initialize with the length
                    for i in range(len(seq)):
                        #pwm_counts.append(np.array([1.0] * 20))
...
def meme_add(meme_out, f, filter_pwm, nsites, trim_filters=False):
    ...
    if ic_start < ic_end:
        print('MOTIF filter%s' % f, file=meme_out)
        if data_type == 'amino':
            print('letter-probability matrix: alength= 20 w= %d nsites= %d' % (ic_end - ic_start + 1, nsites),
                  file=meme_out)
            for i in range(ic_start, ic_end + 1):
                print('%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f '
                      '%.4f' % tuple(filter_pwm[i]), file=meme_out)
        else:
...
def meme_intro(meme_file, seqs):
    ...
    if data_type == 'amino':
        nt_freqs = [nt_counts[i] / nt_sum for i in range(20)]
    else:
...
def plot_filter_heat(param_matrix, out_pdf):
    ...
    plt.figure(figsize=(param_matrix.shape[1], 10))
    ...
    if data_type=='RNA':
        ...
        ax.set_yticklabels('ACGT', rotation='horizontal', size=10) # , size=10)
    plt.savefig(out_pdf)
    plt.close()
...
################################################################################
def plot_filter_logo(filter_outs, filter_size, seqs, out_prefix, raw_t=0, maxpct_t=None, fil_num=0):
    print('filter_outs.shape: ' + str(filter_outs.shape))
    if maxpct_t:
        all_outs = np.ravel(filter_outs)
        if data_type == 'amino':
            all_outs_mean = output_mean[fil_num]
        else:
    ...
    for i in range(filter_outs.shape[0]):  # 各配列を順々に
        for j in range(filter_outs.shape[1]):  # 1配列の文字を順々に
...
def get_motif_fig(filter_weights, filter_outs, out_dir, seqs, sample_i = 0):
    ...
    filters_ic = []
    meme_out = meme_intro('%s/filters_meme.txt'%out_dir, seqs)    #  MEMEファイルの最初(versionやbackgroundsなど)を記述する
    ...
    for f in range(num_filters):
        ...
        #plot_filter_logo(filter_outs[:,f,:], filter_size, seqs, '%s/filter%d_logo'%(out_dir,f), maxpct_t=0.5, fil_num=f)
    ...
    if data_type=='RNA':
        subprocess.call(tomtom_dir + ' -dist pearson -thresh 0.05 -oc %s/tomtom %s/filters_meme.txt %s' % (
        out_dir, out_dir, 'motifs_database/Ray2013_rbp_RNA.meme'), shell=True)
    if data_type=='amino':
        subprocess.call(tomtom_dir + ' -dist pearson -thresh 0.05 -oc %s/tomtom %s/filters_meme.txt %s' % (
        out_dir, out_dir, 'motifs_database/prosite2021_04.meme'), shell=True)
    else:
        subprocess.call(tomtom_dir+' -dist pearson -thresh 0.05 -oc %s/tomtom %s/filters_meme.txt %s' % (out_dir, out_dir, 'motifs_database/myJaspar.meme'), shell=True)
    ...
    if data_type=='RNA':
        ...
        filter_names = name_filters(num_filters, '%s/tomtom/tomtom.txt'%out_dir, 'motifs_database/myJaspar.meme')
    else:
        filter_names = name_filters(num_filters, '%s/tomtom/tomtom.txt' % out_dir, 'motifs_database/prosite2021_04.meme')
    #################################################################
    ...
    for f in range(num_filters):
        ...
        row_cols = (f, consensus, annotation, filters_ic[f])
        #row_cols = (f, consensus, annotation, filters_ic[f], fmean, fstd)
        print( '%-3d  %19s  %10s  %5.2f' % row_cols, file=table_out)
...
def get_motif(filter_weights_old, filter_outs, testing, y = [], index = 0, dir1 = 'motifs/',emb=True,data='DNA',kmer=5,
              s=1,tomtom='meme-5.0.3/src/tomtom', out_mean=[]):
    ...
    if not os.path.isdir(out_dir):
        os.mkdir(out_dir)
    if not os.path.isdir(out_dir + '/tomtom'):
        os.mkdir(out_dir + '/tomtom')
    if index == 0:
        get_motif_fig(filter_weights, filter_outs, out_dir, testing, sample_i)


#  prosite のデータ(https://ftp.expasy.org/databases/prosite/prosite_alignments_for_logos.tar.gz)
    #  からタンパク質のmemeを作成する
def make_meme(testing, dir1='motifs_database', data='amino', dir2='prosite_alignments_for_logos'):
    out_dir = dir1
    global data_type
    data_type = data
    if not os.path.isdir(out_dir):
    ...
    meme_out = meme_intro('%s/prosite.meme' % out_dir, testing)  # MEMEファイルの最初(versionやbackgroundsなど)を記述する
    filters_ic = []
    files = glob.glob(dir2 + '/*')
    for file in files:
        print(file)
        # make a PWM for the filter
        filter_pwm, nsites = make_filter_pwm(file)
        ...
        if nsites < 10:
            ...
            # compute and save information content
            filters_ic.append(info_content(filter_pwm))
        motif = file.replace(dir2 + '/', '').replace('.msa', '')
        motif = motif + ' ' + '(' + motif + ')'

        # add to the meme motif file
        meme_add(meme_out, motif, filter_pwm, nsites, False)

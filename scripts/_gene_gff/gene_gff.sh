...
# There are cases where there are two or more genes with the same name when synonyms are included.
# Therefore, for such genes, check the details of the genes in the database to determine which gene is which,
# and perform the following operation to delete the one that does not match from the file.
# This is tedious, so it is recommended to use the original file.

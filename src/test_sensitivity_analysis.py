...
from src.dataloadars import test_dataLoad
from src.func import ans_one_hot, pad_collate, initialize, convert, NewsDataset, Embedding
from src.layers import myModel
...
def main(args):
    ...
    mer = int(args.mer)#5
    amino_mer = int(args.amino_mer)#4
    amino_emb_dim = int(args.amino_emb_dim)#100
    dna_emb_dim = int(args.dna_emb_dim)#50
    embedding = str(args.embedding)#'embedding'  # 'one_hot'
    save_path = f'{save_path}/mer-{mer}_amer-{amino_mer}_emb-dim-{dna_emb_dim}_aemb-dim-{amino_emb_dim}_embedding-{embedding}'
    ...
    print(f'mer: {mer}')
    print(f'amino_mer: {amino_mer}')
    print(f'dna_emb_dim: {dna_emb_dim}')
    print(f'amino_emb_dim: {amino_emb_dim}')
